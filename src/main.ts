...
let renderer: THREE.WebGLRenderer, camera: THREE.PerspectiveCamera, scene: THREE.Scene, clock: THREE.Clock, gui, stats: Stats;
let environment: THREE.Group, collider: THREE.Mesh, visualizer: MeshBVHVisualizer, player: THREE.Mesh, controls: OrbitControls;
...
function init() {
	...
	(player as any).capsuleInfo = {
		radius: 0.5,
		segment: new THREE.Line3( new THREE.Vector3(), new THREE.Vector3( 0, - 1.0, 0.0 ) )
	};
	player.castShadow = true;
	player.receiveShadow = true;
	(player.material as any).shadowSide = 2;
	scene.add( player );
	reset();
	...
	physicsFolder.add( params, 'gravity', - 100, 100, 0.01 ).onChange( v => {
		...
		params.gravity = parseFloat( String(v) );
	...
	new GLTFLoader().load( '../assets/scene.gltf', res => {
		...
		gltfScene.traverse( c => {
			...
			if (
				...
				(c as any).material && (c as any).material.color.r === 1.0
			) {
		...
		const mergedGeometry: any = staticGenerator.generate();
		mergedGeometry.boundsTree = new MeshBVH( mergedGeometry );

		collider = new THREE.Mesh( mergedGeometry );
		(collider.material as any).wireframe = true;
		(collider.material as any).opacity = 0.5;
		(collider.material as any).transparent = true;
